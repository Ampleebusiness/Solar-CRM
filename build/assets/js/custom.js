...
---------------------------------------------------------------------------------------------*/
	...
	function shop_product_price(){
		var $ex = jQuery('#ex2');
		if ($ex.length && typeof $ex.slider === 'function') {
			$ex.slider({});
		}
	}
...
---------------------------------------------------------------------------------------------*/
	jQuery(window).on('load', function () {
		// Hide preloader first — must not depend on other inits (they can throw if DOM missing)
		page_loader();

		try {
			//  Shop Product Price Range Slider — only when shop filter slider exists
			shop_product_price();
			// > TouchSpin box function by  = jquery.bootstrap-touchspin.js =============== //
			input_number_vertical_form();
			// Bootstrap Select box function by  = bootstrap-select.min.js
			...
			if (window.console && console.error) {
				console.error('custom.js window load init:', err);
			}
			page_loader();
		}
});
	...
	function page_loader() {
		var $loader = jQuery('.loading-area');
		if ($loader.length) {
